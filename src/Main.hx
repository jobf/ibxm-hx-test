...
class Main extends app.App {
	...
	public function start() {
		...
		// for debug
		// player.setAudioSource(new audio.IReplaySource.SineSource(player.getSamplingRate()));
		...
		// writeLine("or click for yesod ;)", "");
		// window.onMouseDown.add((x, y, button) -> {
		// 	if (!Replay.isInitialised) {
		// 		clear();
		...
		// 			var module = new js.lib.Int8Array(request.response);
		// 			loadModule(module);
		// 		}
	...
	function loadModule(data:ModuleFormat) {
		var error = Replay.initialise(data, Std.int(player.getSamplingRate()));
		if (error.length > 0) {
		...
		instruments = [
			for (i in 1...17) {
				...
				var a = StringTools.rpad('$label' + Replay.getInstrumentName(instrument), " ", 24);
				...
				if (instrument <= 0x1f) {
					var label = StringTools.lpad('$instrument', "0", 2);
					b = StringTools.rpad('$label' + Replay.getInstrumentName(instrument), " ", 24);
				}
	...
	function initUi():Void {
		...
		progress = add_button(progressChars.join(""), (text, char) -> {
			...
			Replay.seek(samplePosition); // seek is already camelCase-friendly
